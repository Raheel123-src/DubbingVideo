...
def translate_segments(segments, target_lang):
    ...
    for seg in segments:
        ...
        try:
            ...
            translated_text = response.choices[0].message.content.strip() if response.choices else "translation failed"
...
@web_app.post("/transcribe-dub/")
async def transcribe_and_translate(
    video: UploadFile = File(...),
    lang: str = Form(...)
):
    try:
        file_ext = os.path.splitext(video.filename or "")[1]
...
@web_app.get("/")
async def root():
    return {"message": "NewDubber API is running! Use POST /transcribe-dub/ to process videos."}
